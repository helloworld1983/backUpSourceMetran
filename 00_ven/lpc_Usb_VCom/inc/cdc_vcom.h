...
/**
 * @brief   Check if Vcom is disconnected
 * @return  Returns non-zero value if disconnected.
 */
static INLINE uint32_t vcom_disconnected(void)
{
   g_vCOM.tx_flags = g_vCOM.tx_flags & (!VCOM_TX_CONNECTED);
   return g_vCOM.tx_flags;
}
