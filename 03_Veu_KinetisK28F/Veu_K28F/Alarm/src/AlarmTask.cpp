...
{ 
    ...
    DEBUG_ALARM("StackSize ENTER ALARM: %d \n", uxHighWaterMark);
...
//    ASSERTION (controlPtr != NULL);

    while(FOREVER)
    {
        DEBUG_ALARM("Alarm task \n");
        //test send event to pd task
        DEBUG_ALARM("alarm send event 01 \n");
        PdTask::S_GetInstance()->S_SetEventFlag(START_INH_EVENT_FLAG);
...
//        monPtr->MonitorEnd();
        ...
        uxHighWaterMark = uxTaskGetStackHighWaterMark( NULL );
        DEBUG_ALARM("StackSize REMAIN ALARM: (%d)  USE (%d) \n", uxHighWaterMark, (ALARM_TASK_SIZE - uxHighWaterMark)*4);
