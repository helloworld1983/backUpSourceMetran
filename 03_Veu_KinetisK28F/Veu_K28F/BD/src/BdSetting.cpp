...
void BdSetting::SetPendingValue (LONG value)
{
    ...
    // Notify MgrBdSetting to add this setting to a list of settings to
    // be phased in on the next breath phase boundary.
    (MgrBdSetting::S_GetInstance())->AddSettingToChangeList(this);
    // Check if this setting is associated with a Trigger and if the parameter
    // change should be phased in now.
