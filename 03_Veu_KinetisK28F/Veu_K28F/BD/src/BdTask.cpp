...
BdQEntry BdTask::bdQEntry = { eNoBdEventId };
...
//    DEBUG_BD("%d %d \n", 2 * (BD_Q_MSG_SIZE + 1), BD_Q_MSG_SIZE); //94 x 46
...
//
/******************************************************************************/
STATUS BdTask::Send (BdSingleSettingChangeEvent& singleSetEvent, UNSIGNED suspend)
{
...
{
    const int FOREVER = 1;
    volatile UBaseType_t uxHighWaterMark;uxHighWaterMark = uxTaskGetStackHighWaterMark( NULL );
    DEBUG_BD("StackSize ENTER BD: %d \n", uxHighWaterMark);
    //    Monitor *monPtr = NULL;
...
//    {
//        DEBUG_BD("Receive common settings\n");
...
//    {
//        DEBUG_BD("Receive the ventilation type specific settings\n");
...
//    unsigned int DataIOReadingTimer = 0;
...
//    BdTask::S_GetInstance()->Send(E);
    ...
    while (FOREVER)
    {
        ...
        DEBUG_BD("BD Task send event 10 \n");
        PdTask::S_GetInstance()->S_SetEventFlag(O2_SETTING_EVENT_FLAG);
...
//                    MgrFlowSens::AirFlowSensor->GetCurrentReading();
//                    MgrFlowSens::O2FlowSensor->GetCurrentReading();
        //  Read pressure sensor
//                    PressureSensor::InhPressSensor->GetLastReading();
//                    PressureSensor::ExhPressSensor->GetLastReading();
...
//                        ProxyFlowSensor::S_GetInstance()->CommunicateProxySensor();
//                    }
//                    ProxyStatus Status = ProxyFlowSensor::S_GetInstance()->getLastStatus();
//                    if((Status ==eCalibrated)||(Status==eResetAndRecovered))
//                    if(ProxyFlowSensor::S_GetInstance()->getLastStatus()==eCalibrated)
//                    {
//                        ProxyFlowSensor::S_GetInstance()->GetCurrentReading();
//
//                    }
//                    ProxyStatus currentState = ProxyFlowSensor::S_GetInstance()->getCurrentStatus(); // Need to work ... not debug only
//                }
...
//        phaseMgrPtr->ProcessBdCycle ();
        ...
        if(status == Q_SUCCESS)
        {
        	DEBUG_BD("bdQEntry.Event = %d %d\n", bdQEntry.Event.BatchSettingsChangeEvent.Id);
        }
...
//                }
        ...
        DEBUG_BD("StackSize REMAIN BD: (%d)  USE (%d)\n", uxHighWaterMark, (BD_TASK_SIZE - uxHighWaterMark)*4);
    }
}   // end Entry()

/******************************************************************************/
//$COMMON.OPERATION$
//    Operation Name:ReadDInput()
//
//    Processing:
//
//
...
//
/******************************************************************************/
void BdTask::ReadDInput (void)
{
    MgrDiscreteInput::AirPressSwitch->GetStateDirect();
    MgrDiscreteInput::O2PressSwitch->GetStateDirect();
    MgrDiscreteInput::HFODoorSwitch->GetStateDirect();
    MgrDiscreteInput::HFOPistonSwitch->GetStateDirect();
}

/******************************************************************************/
//$COMMON.OPERATION$
//    Operation Name:WriteDOutput()
//
//    Processing:
//
//
...
//
/******************************************************************************/
void BdTask::WriteDOutput (void)
{
    if(MgrDiscreteOutput::InpedanceLine->GetState() != MgrDiscreteOutput::InpedanceLine->GetDesiredState())
    {
        MgrDiscreteOutput::InpedanceLine->SetStateDirect(MgrDiscreteOutput::InpedanceLine->GetDesiredState());
    }
    if(MgrDiscreteOutput::SafetyValve->GetState() != MgrDiscreteOutput::SafetyValve->GetDesiredState())
    {
        MgrDiscreteOutput::SafetyValve->SetStateDirect(MgrDiscreteOutput::SafetyValve->GetDesiredState());
    }
    if(MgrDiscreteOutput::ExhPressureZero->GetState() != MgrDiscreteOutput::ExhPressureZero->GetDesiredState())
    {
        MgrDiscreteOutput::ExhPressureZero->SetStateDirect(MgrDiscreteOutput::ExhPressureZero->GetDesiredState());
    }
    if(MgrDiscreteOutput::InhPressureZero->GetState() != MgrDiscreteOutput::InhPressureZero->GetDesiredState())
    {
        MgrDiscreteOutput::InhPressureZero->SetStateDirect(MgrDiscreteOutput::InhPressureZero->GetDesiredState());
    }
    if(MgrDiscreteOutput::PurgeFlow->GetState() != MgrDiscreteOutput::PurgeFlow->GetDesiredState())
    {
        MgrDiscreteOutput::PurgeFlow->SetStateDirect(MgrDiscreteOutput::PurgeFlow->GetDesiredState());
    }
    if(MgrDiscreteOutput::OilPump->GetState() != MgrDiscreteOutput::OilPump->GetDesiredState())
    {
        MgrDiscreteOutput::OilPump->SetStateDirect(MgrDiscreteOutput::OilPump->GetDesiredState());
    }
}
