...
void i2c_Init(I2C_Type* chanelBase, uint32_t baudRate)
{
	...
	if(chanelBase == I2C2)
	{
		...
		status_t status = I2C_RTOS_Init(&master_rtos_handle, I2C2, &masterConfig, sourceClock);
	...
	else if(chanelBase == I2C3)
	{
        ...
        status_t status = I2C_RTOS_Init(&master_rtos_handle, I2C3, &masterConfig, sourceClock);
        if (status != kStatus_Success)
        {
            PRINTF("I2C master: error during init, %d", status);
        }
