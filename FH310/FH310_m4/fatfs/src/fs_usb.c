...
/*2017/09/04 Quyen Ngo Remove Chip_RTC_Enable function*/
...
/* Initialize Disk Drive */
DSTATUS disk_initialize_nonRTOS(BYTE drv)
{
	if (drv) {
		return STA_NOINIT;				/* Supports only single drive */
	}
...
//	Chip_RTC_Enable(LPC_RTC, ENABLE);
	...
	FSUSB_DiskInsertWait_nonRTOS(hDisk); /* Wait for card to be inserted */
...
DRESULT disk_read (BYTE pdrv, BYTE* buff, DWORD sector, UINT count)//Edit by Viet
{
	//if (drv || !count) {
	if (pdrv || !count) {//Edit by Viet
		return RES_PARERR;
	}
	if (Stat & STA_NOINIT) {
		return RES_NOTRDY;
...
DRESULT disk_write (BYTE pdrv, const BYTE* buff, DWORD sector, UINT count)//Edit by Viet
{
