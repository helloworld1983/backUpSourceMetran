...
void Board_UART_Init(LPC_USART_T *pUART)
{
	if (pUART == LPC_USART0) {
		...
		Chip_SCU_PinMuxSet(0x1, 15, (SCU_MODE_PULLDOWN | SCU_MODE_FUNC1));			/* P9.3 : UART2_TXD */
		Chip_SCU_PinMuxSet(0x1, 16, (SCU_MODE_INACT | SCU_MODE_INBUFF_EN | SCU_MODE_ZIF_DIS | SCU_MODE_FUNC1));	/* P1.16 : UART2_RXD */
	}
	else if (pUART == LPC_UART1) {
		Chip_SCU_PinMuxSet(0xC, 2, (SCU_MODE_PULLDOWN | SCU_MODE_FUNC2));			/* PC.2 : UART1_CTS */
		Chip_SCU_PinMuxSet(0xC, 3, (SCU_MODE_PULLDOWN | SCU_MODE_FUNC2));			/* PC.3 : UART1_RTS */
...
void Board_CalibrateTouch(CALIBDATA_T* setup)
{
	...
	if (setup != NULL) {
		...
		r1.x = 15;//setup->refX1;
		r1.y = 465;//setup->refY1;
		r2.x = 400;//setup->refX2;
		r2.y = 80;//setup->refY2;
		r3.x = 785;//setup->refX3;
		r3.y = 465;////setup->refY3;
		s1.x = 221;//setup->scrX1;
		s1.y = 302;//setup->scrY1;
		s2.x = 1982;////setup->scrX2;
		s2.y = 3156;//setup->scrY2;
		s3.x = 3836;//setup->scrX3;
		s3.y = 286;//setup->scrY3;
