...
/* Transmit and Receive data in master mode */
//#0001 2017/09/28 add timeout for while loop
//Q_comment
//length = xfer->txSz+xfer->rsSz
//bitrate = 100000
//Time transfer = 9*(length+1)*bitrate<===>(lenghth+2)*1loopTime
//MaxTimeWaiting is 4*Time transfer
//Maxium Length use by Application is 34==>MaxTimeWaiting = 4*(34+2)*0.0958ms=13.79ms
//#0001 2017/11/20 add timeout for while loop for wait state change.
//Maximum time for wait here is 15ms
#define MAX_WAIT_STATE_CHANGE 100000
uint32_t Chip_I2CM_XferBlocking(LPC_I2C_T *pI2C, I2CM_XFER_T *xfer)
{
	uint32_t ret = 0;
	static uint32_t s_timeCnt = 0;
	uint32_t pckLenghth = xfer->txSz+xfer->rxSz;
	/* start transfer */
	Chip_I2CM_Xfer(pI2C, xfer);
	while (ret == 0) {
		if(s_timeCnt < 4*(pckLenghth+2))
		{
			s_timeCnt++;
			/* wait for status change interrupt */
			volatile uint32_t waitStateCnt = 0;
			while ( Chip_I2CM_StateChanged(pI2C) == 0) {
				waitStateCnt++;
				if(waitStateCnt > MAX_WAIT_STATE_CHANGE)
				{
					waitStateCnt = 0;
					return ret;
				}
