...
uint8_t vcom_Checksum(uint16_t initCRC, unsigned char *pData, long nBytes)
{
	while(nBytes--)
	{
		initCRC ^= *pData++;
...
uint32_t vcom_write(uint8_t *pBuf, uint32_t len)
{
	...
	uint32_t ret = 0;
	uint8_t Buff[len+1];
	memcpy(Buff,pBuf,len);
	uint8_t CRC = vcom_Checksum(0x0000,(unsigned char*)Buff,len);
	Buff[len-1]=CRC;
	Buff[len]='\0';
	if ( (pVcom->tx_flags & VCOM_TX_CONNECTED) && ((pVcom->tx_flags & VCOM_TX_BUSY) == 0) ) {
		...
		/* enter critical section */
		NVIC_DisableIRQ(LPC_USB_IRQ);
		ret = USBD_API->hw->WriteEP(pVcom->hUsb, USB_CDC_IN_EP, Buff, len+1);
		/* exit critical section */
		NVIC_EnableIRQ(LPC_USB_IRQ);
	}
	return ret;
