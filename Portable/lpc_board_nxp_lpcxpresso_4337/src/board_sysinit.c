...
STATIC const PINMUX_GRP_T pinmuxing[] = {
		...
		/* I2S */
		{0x2, 13, (SCU_PINIO_FAST | SCU_MODE_FUNC0)},
		{0x3, 0, (SCU_PINIO_FAST | SCU_MODE_FUNC2)},//Chip_SCU_PinMuxSet(0x3, 0, (SCU_PINIO_FAST | SCU_MODE_FUNC2));
		{0x3, 2, (SCU_PINIO_FAST | SCU_MODE_FUNC0)},//Chip_SCU_PinMuxSet(0x3, 2, (SCU_PINIO_FAST | SCU_MODE_FUNC0));
		{0x3, 1, (SCU_PINIO_FAST | SCU_MODE_FUNC0)},//Chip_SCU_PinMuxSet(0x3, 1, (SCU_PINIO_FAST | SCU_MODE_FUNC0));
};
...
void Board_SetupClockingLow()
{
	int i;

	/* Enable Flash acceleration and setup wait states */
	Chip_CREG_SetFlashAcceleration(60000000);
...
void Board_SystemInit(void)
{
	...
	if((!Chip_GPIO_ReadPortBit(LPC_GPIO_PORT,AC_DETECT_PORT_NUM,AC_DETECT_BIT_NUM) == true)
			&& (!Chip_GPIO_ReadPortBit(LPC_GPIO_PORT,BAT_DETECT_PORT_NUM,BAT_DETECT_BIT_NUM)== false))
	{
		Board_SetupClockingLow();
	}
	else
	{
		Board_SetupClocking();
...
void Board_SystemInitSetClockHigh(void)
{
	/* Setup system clocking and memory. This is done early to allow the
	   application and tools to clear memory and use scatter loading to
	   external memory. */
